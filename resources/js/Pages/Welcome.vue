...
<template>
    ...
    <div class="flex flex-col lg:flex-row min-h-screen">
        ...
        <div class="flex-1 p-4 md:p-8 lg:p-12">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 px-4 sm:px-0">
                <div v-for="project in projectsFiltered" :key="project.id"
                     class="relative bg-white rounded-lg shadow-md hover:shadow-lg transition duration-200 ease-in-out border border-gray-300">
                    <img :src="`/storage/${project.cover_path}`" alt="Project Image"
                    ...
                    <div class="p-3 sm:p-4 md:p-6">
                        ...
                        <h3 class="text-base sm:text-lg md:text-xl font-medium text-gray-800 mb-2 sm:mb-3">
                            {{ project.name }}</h3>
                        <p class="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4 md:mb-6 h-16 sm:h-20 md:h-24 overflow-hidden">
        ...
        <div
            class="w-full sm:w-4/5 md:w-[20%] bg-white p-6 md:p-8 shadow-lg rounded-xl border border-gray-200 hover:shadow-2xl transition-all duration-300">
            ...
            <div v-if="!props.invitations.length"
                 class="flex text-sm flex-col items-center justify-center mt-8 p-8 bg-gray-50 rounded-lg shadow-inner">
            ...
            <!-- Lista de convites -->
            <div v-else>
                <div v-for="invitation in props.invitations" :key="invitation.id"
                     class="bg-gray-50 p-6 rounded-lg shadow-md mb-6 transition-all duration-200 hover:shadow-xl">
        ...
        >
            <ElForm :model="form" label-width="120px" class="p-4">
                ...
                <ElFormItem label="Descrição">
                    <ElInput type="textarea" v-model="form.description" placeholder="Descrição do Projeto"
                             class="w-full"/>
                </ElFormItem>
                <ElFormItem label="Data de Início">
                    <ElDatePicker format="DD/MM/YYYY" value-format="YYYY-MM-DD HH:mm:ss"
                                  v-model="form.start_date" type="date" placeholder="Escolha a data de início"
                                  class="w-full"/>
                </ElFormItem>
                <ElFormItem label="Data Final">
                    <ElDatePicker format="DD/MM/YYYY" value-format="YYYY-MM-DD HH:mm:ss"
                                  v-model="form.end_date" type="date" placeholder="Escolha a data final"
                                  class="w-full"/>
                </ElFormItem>
                ...
                <ElFormItem class="flex flex-col sm:flex-row justify-between gap-2">
                    <ElButton type="primary" @click="handleSubmit"
                              ...
                              :disabled="!isFormValid">Criar Projeto
                    </ElButton>
                    <ElButton @click="isClosed"
